...
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[
    node distance=1.8cm and 5.0cm, % Increased node distances further
    >=Stealth,
    auto,
    box/.style={
        draw,
        rectangle,
        align=center,
        minimum height=1.8cm,  % Increased minimum height
        text width=5.0cm,    % Increased text width
    ...
    ellipse_style/.style={
        ...
        ellipse,
        align=center,
        minimum height=2.5cm,  % Increased minimum height
        text width=4.99cm,  % Increased text width
...
% Nodes
\node[ellipse_style] (tms) {\textbf{Transactive Memory Systems/\\Large Language Models} \\ (Mutual Trust, Shared Mental Models)}; % Added newline

\node[box, below left=of tms] (input) {\textbf{Input}
...
\node[box, below=of tms] (ai_comm) {\textbf{AI-Group Communication}
\begin{itemize}[leftmargin=*, itemsep=0pt, parsep=0pt, topsep=3pt]
...
\node[box, below right=of tms] (group_decision) {\textbf{Group Decision}
 \begin{itemize}[leftmargin=*,itemsep=0pt, parsep=0pt, topsep=3pt]
    \item Human-AI Decision Strategies
    \item Decision Outcomes
\end{itemize}
};
\node[ellipse_style, below=of ai_comm] (context) {\textbf{Context} \\ (Task Characteristics, Cognitive Load)}; % Added newline
